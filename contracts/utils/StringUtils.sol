...
library StringUtils {
    ...
    function toLowerCase(string memory input) public pure returns (string memory out) { 
        assembly {
            ...
            let char := 0 // create variable that will contain the current char input
            len := add(0x20, len)
            for { let i := 0x20 } lt(i, len) { i := add(i, 1) } {
                char := byte(0, mload(add(input, i))) // char = input[i], data offset = 0x20 (1st 32 is reserved for size) + i to pickup the right index
